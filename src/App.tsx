...
function App() {
  ...
  const showPauseButton =
    !isPaused && (phase === 'showing' || phase === 'input' || phase === 'result')
    ...
    <>
      ...
      {isPaused ? (
        <section className="flex w-full max-w-md flex-col items-center gap-6 rounded-[6px] border border-mnemo-border bg-mnemo-cell px-6 py-10 text-center">
          <h2 className="text-2xl font-semibold text-white">Oyun duraklatıldı</h2>
          <p className="text-mnemo-hud">Tamamlanan tur: {roundCount}</p>
          <p className="text-mnemo-hud">
            Son 10 tur ortalaması: %{formattedAverage}
          </p>
          <button type="button" className={buttonClassName} onClick={resumeGame}>
            Devam Et
          </button>
        </section>
      ) : levelComplete === true ? (
        <section className="flex w-full max-w-md flex-col items-center gap-6 rounded-[6px] border border-mnemo-border bg-mnemo-cell px-6 py-10 text-center">
          ...
          <button type="button" className={buttonClassName} onClick={nextLevel}>
            Sonraki Bölüm
          </button>
        </section>
      ) : (
      ...
      <button
        ...
        className="fixed bottom-4 right-4 rounded-[8px] bg-[#2A2A45] p-[10px] text-[#6B6B8A] transition-colors hover:bg-[#3A3A60] active:scale-95"
