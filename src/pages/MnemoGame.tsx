...
import { ONBOARDING_SEEN_KEY } from "../components/Onboarding";
import { AUTH_SHOWN_KEY } from "./Landing";
...
// Mnemo oyun ekranını render eder
export function MnemoGame() {
  ...
  useEffect(() => {
    ...
    clearAuthPrompt();
  }, [authPromptRequested, clearAuthPrompt]);
      ...
      <div className="flex h-svh min-h-0 flex-col overflow-hidden bg-mnemo-bg">
        <header className="mnemo-hud shrink-0 px-4 py-3">
          ...
          <div className="mx-auto flex w-full max-w-lg items-center justify-center overflow-x-auto gap-1.5 text-xs font-medium text-mnemo-hud sm:gap-3 sm:text-sm">
            ...
            <Link
              to="/"
              className="whitespace-nowrap rounded-[6px] border border-mnemo-border px-2 py-1 text-xs transition-colors hover:bg-mnemo-cell-hover hover:text-white"
            >
              Ana Sayfa
            </Link>
          </div>
